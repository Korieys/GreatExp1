...
import { Plus, Edit2, Trash2, Shield, Users as UsersIcon, Check, KeyRound } from 'lucide-react';
...
const AdminPractitioners = () => {
    ...
    const isSuperAdmin = userPermissions === null;
                                        ...
                                        <tr key={user.uid} className="hover:bg-slate-50/50 transition-colors">
                                            <td className="px-8 py-6 whitespace-nowrap">
                                                ...
                                                <div className="flex flex-col gap-2 mt-2">
                                                    {!user.permissions && (
                                                        <span className="w-fit px-3 py-1 bg-amber-50 border border-amber-200 text-amber-600 text-[10px] font-black uppercase tracking-widest rounded-md">Super Admin (All Access)</span>
                                                    )}
                                                    {isSuperAdmin && (
                                                        <button 
                                                            onClick={() => handlePasswordReset(user.email)} 
                                                            className="w-fit px-3 py-1.5 flex items-center gap-1.5 bg-slate-50 border border-slate-200 text-slate-600 hover:bg-slate-100 hover:text-slate-900 rounded-lg text-xs font-bold transition-colors shadow-sm"
